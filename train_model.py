...
***
*** IMPROVEMENT 3 (FINAL): The "best" model is now saved based on a
*** balanced score: (val_cascade_f1 + val_node_f1) / (val_time_mae + 1e-6),
*** which optimizes for detection AND causal path accuracy.
"""
...
class Trainer:
    ...
    def __init__(
        ...
        self.start_epoch = 0
        # ====================================================================
        # START: IMPROVEMENT - Save best model based on BALANCED SCORE
        # ====================================================================
        self.best_val_score = 0.0 # We want to MAXIMIZE this score
    ...
    def load_checkpoint(self, checkpoint_path: str):
        ...
        # ====================================================================
        # START: IMPROVEMENT - Load best balanced score
        # ====================================================================
        self.best_val_score = checkpoint.get('val_score', 0.0) # <-- CHANGED
        ...
        print(f"✓ Resumed from epoch {self.start_epoch} (best val_score: {self.best_val_score:.4f})")
    ...
    def train(self, num_epochs: int, early_stopping_patience: int = 10):
        ...
        for epoch in range(self.start_epoch, num_epochs):
            ...
            # START: IMPROVEMENT - Save best model based on BALANCED SCORE
            # ====================================================================
            # We want high F1 (detection) and low MAE (timing).
            # We define a new score to MAXIMIZE: (Cascade F1 + Node F1) / (Timing MAE + epsilon)
            # We add 1e-6 to MAE to prevent division by zero if MAE is 0.0
            # We only check this if time_mae is valid (e.g., > 0)
            
            current_val_score = 0.0
            if val_metrics['time_mae'] > 0 and (val_metrics['cascade_f1'] > 0 or val_metrics['node_f1'] > 0):
                current_val_score = (val_metrics['cascade_f1'] + val_metrics['node_f1']) / (val_metrics['time_mae'] + 1e-6)
            ...
            if current_val_score > self.best_val_score:
                ...
                torch.save({
                    ...
                    'val_score': current_val_score, # <-- ADDED
                    'cascade_threshold': self.cascade_threshold,
                    'node_threshold': self.node_threshold,
                    'history': self.history
                }, f"{self.output_dir}/best_model.pth")
                print(f"  ✓ Saved best model (New Score: {current_val_score:.4f}, F1s: {val_metrics['cascade_f1']:.3f}/{val_metrics['node_f1']:.3f}, MAE: {val_metrics['time_mae']:.3f}m)")
            
            # Fallback to save by val_loss if no cascades were in the validation set (score is 0)
            elif val_metrics['loss'] < self.best_val_loss and self.best_val_score == 0.0:
                self.best_val_loss = val_metrics['loss']
                patience_counter = 0
                print(f"  ✓ Saved best model (val_loss: {val_metrics['loss']:.4f}) - (No valid timing/F1 metrics yet)")
        ...
        print(f"  Best validation score: {self.best_val_score:.4f} ((cF1+nF1) / MAE)")
...
class Trainer:
    ...
    def train(self, num_epochs: int, early_stopping_patience: int = 10):
        ...
        for epoch in range(self.start_epoch, num_epochs):
            ...
            current_val_score = 0.0
            # Only calculate score if we have valid timing and f1 scores
            if val_metrics['time_mae'] > 0 and (val_metrics['cascade_f1'] > 0 or val_metrics['node_f1'] > 0):
                current_val_score = (val_metrics['cascade_f1'] + val_metrics['node_f1']) / (val_metrics['time_mae'] + 1e-6)
